...
- Before a release: `claude plugin validate .`, a local `claude plugin marketplace add <path>` + install, the same with `codex plugin marketplace add <path>`, and a manual `delegate-task` run in each CLI against staging.
